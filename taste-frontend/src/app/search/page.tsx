	...
	const loadNextPage = () => {
		...
		if (response) {
			fetch(response.next).then((res) => {
				res.json().then((data) => {
					setResponse({
						count: response.count + data.count,
						next: data.next,
			...
			<div className="h-screen  flex flex-col dark:bg-neutral-900">
				...
				>
					<SearchBar
						...
						className=" bg-gray-100 placeholder-slate-400 outline-none ring-0 rounded-full flex px-10 py-3 mt-3 dark:bg-neutral-700 dark:text-zinc-300"
					/>
				...
				<div
					className={`w-full  md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6   grid-flow-row ${
						response && response.results.length > 0 ? "grid-cols-2" : ""
					} gap-3 px-3  mb-5 `}
						...
						response.results.length > 0 ? (
							...
							<div className=" flex justify-center flex-col items-center">
								<div className=" text-center p-3">
									Looks like your search didn&apos;t return any products
								</div>
								<div className=" text-center p-3 dark:text-neutral-500 text-neutral-400">
				...
				{response && response.next && response.results.length > 0 ? (
					<SpringContainer
						className=""
